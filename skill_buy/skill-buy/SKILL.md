...
用户说: "帮我买一台 ubuntu 8核32G 的服务器"
  ...
  Step 2: 一键购买 → buy
    ├─ 加载配置 + AK/SK
    ├─ 如缺少镜像/规格 → 进入交互式选择（查询API展示可选列表）
    ├─ 首次使用 → 提示设置登录密码（可选记住密码到配置文件）
    ├─ 自动填充子网/安全组(如为空)
...
| 场景 | 行为 |
|------|------|
| **首次购买** (adminPass 为空) | 提示用户输入或自动生成密码，询问是否保存 |
| **保存后再次购买** | 直接使用已保存的密码，无需重新输入 |
| **每次换新密码** | 在提示时选择"不保存"，下次会重新询问 |

**交互示例：**
```
  选择方式: [1) 手动输入密码  2) 自动生成] (默认: 2): 2
  已自动生成密码: Xk9#mP2vRn1q
  确认使用此密码? (y/n, 默认 y): y
  是否将此密码保存到配置文件中？... (y/n): y
  [OK] 密码已保存到配置文件，下次购买将直接使用
```
...
| adminPass | 首次购买时**交互式设置**（可保存到JSON） | 8-32位，含字母/数字/特殊字符 |

**总结：用户真正需要手动设置的只有 AK/SK 和首次密码，其余均可自动化！**

### 交互式配置选择

当 `buy` 命令执行时发现关键配置缺失（如镜像、规格），会自动进入交互模式：

```bash
python scripts/buy_bcc.py buy
# 如果 imageId 或 spec 为空：
# → 自动查询 API 展示可选镜像列表 (CentOS/Ubuntu/Windows 等)
# → 自动查询 API 展示可用规格列表 (按 CPU/内存排序)
# → 用户输入编号选择 → 自动回写配置文件
```

**特点：**
- 按操作系统分组显示镜像（CentOS/Ubuntu/Debian/Windows 等）
- 规格支持翻页浏览（每页20个）
- 选择后自动回写到 `bcc_config.json`，下次直接使用
...
pwd = info["access"]["password"]               # <YOUR_PASSWORD>
```

## Resources

### scripts/
- **buy_bcc.py**: 核心脚本，集成完整功能：
  - **密码管理**：首次设置、记住到JSON、下次自动使用
  - **交互式选择**：镜像/规格缺失时展示API查询结果供用户选择
  - `resolve` — 一键智能匹配镜像+规格
  - `resolve-image` — 单独智能匹配镜像
  - `resolve-spec` — 单独智能匹配规格
  - `buy` — 执行购买（含交互式配置 + 自动子网/安全组解析）
  - `query-images` / `query-specs` / `query-resources` / `show-config`

### assets/
- **bcc_config.json**: 配置文件，运行时被自动回写（包含 adminPass 密码记忆）
