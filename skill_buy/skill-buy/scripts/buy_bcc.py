...
_PASSWORD_SAVE_PROMPT = """
是否将此密码保存到配置文件中？
保存后下次购买时将直接使用此密码，无需再次输入 (y/n): """


def _write_password_to_config(password: str, config_path: Optional[str] = None) -> bool:
    """将密码写入 JSON 配置文件的 adminPass 字段"""
    ...
    try:
        ...
        print(f"  [WARN] 保存密码失败: {e}")
        return False


def resolve_password(config: Dict[str, str], config_path: Optional[str] = None) -> str:
    """解析服务器管理员密码（支持交互式输入和记忆功能）

    逻辑：
    1. 如果配置文件中已有 adminPass 且非空 → 直接使用，不询问
    2. 如果为空 → 提示用户创建/输入密码
       - 可选择自动生成或手动输入
       - 询问是否记住密码（写入 JSON）
    """
    saved_pass = config.get("admin_pass", "").strip()

    # 情况1：已有保存的密码，直接使用
    if saved_pass:
        print(f"\n  [INFO] 使用已保存的密码: {mask_password(saved_pass)}")
        return saved_pass
    ...
    print("\n  请为新服务器设置管理员登录密码:")
    print("    - 长度 8-32 位")
    print("    - 必须包含字母、数字和特殊字符 (!@#$%^*())")

    while True:
        choice = input("\n  选择方式: [1) 手动输入密码  2) 自动生成] (默认: 2): ").strip() or "2"
        ...
        if choice == "1":
            ...
            if len(pwd) < 8 or len(pwd) > 32:
                print(f"  [ERROR] 密码长度需在 8-32 位之间 (当前: {len(pwd)} 位)")
                continue
            if not any(c.isalpha() for c in pwd):
                print("  [ERROR] 密码必须包含字母")
                continue
            if not any(c.isdigit() for c in pwd):
                print("  [ERROR] 密码必须包含数字")
                continue
            if not any(c in "!@#$%^*()" for c in pwd):
                print("  [ERROR] 密码必须包含特殊字符 (!@#$%^*())")
                continue
            ...
            print(f"\n  已自动生成密码: {pwd}")
            confirm = input(f"  确认使用此密码? (y/n, 默认 y): ").strip().lower()
    ...
    # 询问是否保存密码
    save_choice = input(_PASSWORD_SAVE_PROMPT).strip().lower()
    if save_choice == "y" or save_choice == "yes" or save_choice == "":
        if _write_password_to_config(pwd, config_path):
            print(f"  [OK] 密码已保存到配置文件，下次购买将直接使用")
        config["admin_pass"] = pwd  # 更新内存中的配置
    else:
        print(f"  [INFO] 密码未保存，下次购买需要重新设置")
...
def _interactive_select_image(client: BccClient) -> Tuple[str, str]:
    """交互式选择镜像：查询可用镜像并让用户选择"""
    resp = client.list_images()
    images = body_map(resp).get("images", [])
    ...
    for cat in sorted(categories.keys()):
        ...
        for item in items[:8]:
            ...
            print(f"  [{idx:>2}] {item['id']:<18}  {item['name']}")
        if len(items) > 8:
            print(f"       ... 还有 {len(items)-8} 个")

    while True:
        try:
            choice = input(f"\n  请输入编号 (1-{len(option_list)}): ").strip()
            if not choice:
                print("  [ERROR] 请选择一个编号")
                continue
            sel_idx = int(choice) - 1
            if 0 <= sel_idx < len(option_list):
                selected = option_list[sel_idx]
                print(f"\n  [OK] 已选择: {selected['id']} ({selected['name']})")
                return selected["id"], selected["name"]
            else:
                print(f"  [ERROR] 编号超出范围 (1-{len(option_list)})")
        except ValueError:
            print("  [ERROR] 请输入数字编号")


def _interactive_select_spec(client: BccClient, zone: str) -> Dict[str, Any]:
    """交互式选择实例规格：查询可用规格并让用户选择"""
    resp = client.list_flavor_spec(zone_name=zone)
    data = body_map(resp)
    zone_resources = data.get("zoneResources") or []
    ...
    print("  请选择实例规格:")
    print("=" * 60)
    print(f"\n  {'编号':<4} {'规格':<24} {'CPU':>4} {'内存':>6}")
    print("  " + "-" * 45)

    page_size = 20
    total_pages = (len(all_specs) + page_size - 1) // page_size
    current_page = 1

    def show_page(page: int) -> None:
        start = (page - 1) * page_size
        end = min(start + page_size, len(all_specs))
        for i in range(start, end):
            s = all_specs[i]
            print(f"  [{i+1:>3}] {s['spec']:<24} {s['cpu']:>4}C {s['mem']:>4}GB")
        if total_pages > 1:
            print(f"\n  第 {page}/{total_pages} 页 (共 {len(all_specs)} 个规格)")

    show_page(current_page)

    while True:
        prompt = f"\n  请输入编号 (1-{len(all_specs)})"
        if total_pages > 1:
            prompt += " | 输入 n 下一页 | p 上一页"
        prompt += ": "
        ...
        if choice == "n" and current_page < total_pages:
            ...
            print("  [ERROR] 请选择一个编号或翻页")
            continue

        try:
            sel_idx = int(choice) - 1
            if 0 <= sel_idx < len(all_specs):
                selected = all_specs[sel_idx]
                print(f"\n  [OK] 已选择: {selected['spec']} ({selected['cpu']}核{selected['mem']}G)")
                return selected
            else:
                print(f"  [ERROR] 编号超出范围 (1-{len(all_specs)})")
        except ValueError:
            print("  [ERROR] 请输入有效命令")


def interactive_config_setup(config: Dict[str, str], config_path: Optional[str] = None) -> Dict[str, str]:
    """运行时交互式配置：当关键配置缺失时引导用户选择"""
    updated = dict(config)
    client = build_client(updated)

    # 镜像选择
    if not updated.get("image_id"):
        print("\n[CONFIG] 未设置镜像，请选择操作系统:")
        image_id, image_name = _interactive_select_image(client)
        updated["image_id"] = image_id
        updated["image_name"] = image_name
        _write_config_image(image_id, image_name, config_path)

    # 规格选择
    if not updated.get("spec_id"):
        print("\n[CONFIG] 未设置规格，请选择实例类型:")
        selected = _interactive_select_spec(client, updated["zone"])
        spec_name = selected["spec"]
        updated["spec_id"] = spec_name
        _write_config_spec(spec_name, config_path)
...
def do_buy(config: Dict[str, str], config_path: Optional[str] = None, *, interactive: bool = True) -> int:
    """执行购买流程

    Args:
        config: 配置字典
        config_path: 配置文件路径
        interactive: 是否在配置缺失时启用交互式选择（默认 True）
    """

    # 如果启用交互模式且关键配置缺失，引导用户选择
    if interactive and (not config.get("image_id") or not config.get("spec_id")):
        config = interactive_config_setup(config, config_path=config_path)
    ...
    print(f"  规格:       {config.get('spec_id', '(待查询)')}")
    ...
    # 密码处理（交互式：首次设置 / 已保存则直接使用）
    admin_pass = resolve_password(config, config_path=config_path)
